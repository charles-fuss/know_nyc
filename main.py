...
# reconstructing the data as a dictionary
    ...
    geojson = {
        ...
        "features": [
        {
            ...
            "geometry" : {
                "type": "Point",
                "coordinates": [data[d]["coords"][1], data[d]["coords"][0]],
                },
            "properties" : {
                'name' : d,
            },
...
def subways(address):
    clear_json('nearby_subways.json')
    f = open("nearby_subways.json", "a")
    sub_list = {}
    hold_error = True
    user_data = find_address(address, False)
    ...
    remove_dups = set()
    valid_addresses = {}
    # CHANGED VALID_ADDRESSES FROM SET TO A DICT. BROKEN ATM
    for key in sub_list:
        ...
        if dist < .5:
                if key[0:len(key)-9] not in remove_dups:
                    ...
                    # FIX THE DICTIONARY VALID_ADDRESSES, RIGHT NOW THE KEYS ARE FUCKED
                    valid_addresses['subway'] = key[train_address]['line']
                    valid_addresses['subway']['coords'] = key['coords']
    print(valid_addresses)              
    toGeoJSON('nearby_subways.json', 'GEO_nearby_subways.json')
...
def parks(address):
    ...
    # f.write(json.dumps(nearby_parks, indent = 4))  # write json to nearby_parks
    json.dump(nearby_parks, open("parks_json.json",'w'))
    toGeoJSON('parks_json.json', 'parks_GEOJSON.json')
    return 'finished parks'
